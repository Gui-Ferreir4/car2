...
def analisar(df, modelo, combustivel, valores_ideais):
    ...
    try:
        faixa_ideal = valores_ideais[modelo][combustivel].get("SHRTFT1pct", faixa_ideal)
    except KeyError:
        pass

    status = "OK" if faixa_ideal["min"] <= media <= faixa_ideal["max"] else "Alerta"
    mensagem = "Curto prazo de ajuste de combustível dentro dos padrões." if status == "OK" \
        else "Ajuste de combustível em curto prazo fora da faixa ideal. Verifique sensores ou mistura."
